...
const parts = Array.from({length:13}, (_,i) =>
  fs.readFileSync(`payload/p${String(i).padStart(2,'0')}.txt`, 'utf8')
);
let html = zlib.gunzipSync(Buffer.from(parts.join(''), 'base64')).toString('utf8');

// Ajustes 4.2.1: clave de inmueble obligatoria y recuperación de cambios locales.
html = html.replace(
...
// Ajustes 4.2.2/4.2.3: navegación móvil completa y compatible.
html = html.replace(
...
`  const bottom=document.getElementById('bottomNav');
  const mobile=NAV.filter(x=>['dashboard','properties','leases','payments','expenses'].includes(x[0]));
  bottom.innerHTML=mobile.map(([id,ico,label])=>\`<button data-view="${id}" class="${currentView===id?'active':''}"><span class="nav-icon">${ico}</span>${label}</button>\`).join('');
  bottom.querySelectorAll('button').forEach(b=>b.onclick=()=>navigate(b.dataset.view));`,
`  const bottom=document.getElementById('bottomNav');
...
// El repositorio publica SVG; sustituimos referencias PNG heredadas.
